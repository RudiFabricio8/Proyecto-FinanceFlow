...
export class StorageService {
  ...
  saveUser(user: User, rememberMe: boolean = false): void {
    ...
    if (user.organizationId) {
      localStorage.setItem(this.KEYS.ORGANIZATION_ID, user.organizationId);
    }
    if (user.organizationName) {
      localStorage.setItem(this.KEYS.ORGANIZATION_NAME, user.organizationName);
    }
  ...
  getUser(): Partial<User> | null {
    ...
    const organizationId = localStorage.getItem(this.KEYS.ORGANIZATION_ID);
    const organizationName = localStorage.getItem(this.KEYS.ORGANIZATION_NAME);
  ...
  clearUser(): void {
    ...
    localStorage.removeItem(this.KEYS.ORGANIZATION_ID);
    localStorage.removeItem(this.KEYS.ORGANIZATION_NAME);
